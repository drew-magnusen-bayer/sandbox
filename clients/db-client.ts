...
export class DataBaseAccess {
  ...
  static async getDailyStagingReportDetails() {    
    try {
      await this.createClient();
      await this.connectClient();

      const ftsTotalCountQuery = `SELECT count(*) FROM seed_operations.staged_sets where source = 'fts';`;
      const ftsAddedCountQuery = `SELECT count(*) FROM seed_operations.staged_sets where source = 'fts' and created_at > current_date - 1;`;
      const ftsIngestedCountQuery = `Select Count(*) from 
      (SELECT set_id 
        FROM seed_operations.plot_bids 
        where source = 'fts' and created_at > current_date - 1 
        group by set_id
      ) as ingestedSets;`;

      const velocityTotalCountQuery = `SELECT count(*) FROM seed_operations.staged_sets where source = 'velocity';`;
      const velocityTotalPreIngestedCountQuery = `SELECT count(*) FROM seed_operations.staged_sets where source = 'velocity' and ingested_prematerial = true;`;
      const velocityAddedCountQuery = `SELECT count(*) FROM seed_operations.staged_sets where source = 'velocity' and created_at > current_date - 1;`;
      const velocityIngestedCountQuery = `Select Count(*) from 
      (SELECT set_id 
        FROM seed_operations.plot_bids 
        where source = 'velocity' and created_at > current_date - 1 
        group by set_id
      ) as ingestedSets;`;
      
      const ftsTotalCount = await this.client.query(ftsTotalCountQuery);
      const ftsAddedCount = await this.client.query(ftsAddedCountQuery);
      const ftsIngestedCount = await this.client.query(ftsIngestedCountQuery);
      
      const velocityTotalCount = await this.client.query(velocityTotalCountQuery);
      const velocityTotalPreIngestedCount = await this.client.query(velocityTotalPreIngestedCountQuery);
      const velocityAddedCount = await this.client.query(velocityAddedCountQuery);
      const velocityIngestedCount = await this.client.query(velocityIngestedCountQuery);

      const stagingDetails: IDailySetIngestionReportDetails = {
        fts: {
          totalStagingCount: ftsTotalCount.rows[0].count,
          addedCount: ftsAddedCount.rows[0].count,
          ingestedCount: ftsIngestedCount.rows[0].count
        },
        velocity: {
          totalStagingCount: velocityTotalCount.rows[0].count,
          totalPreIngested: velocityTotalPreIngestedCount.rows[0].count,
          addedCount: velocityAddedCount.rows[0].count,
          ingestedCount: velocityIngestedCount.rows[0].count
        }
      };
    
      return stagingDetails;
    } catch (error) {
      console.warn(`Error with getDailyStagingReportDetails. Error: ${error}`);
    } finally {
      this.client.end();
    }
  }
